...
class EccPair(width: Int) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val errorLocation = Input(UInt(log2Ceil(width + 1).W))
    val syndromeOut = Output(UInt(eccBits.W))
    val injectSecondError = Input(Bool())
    val secondErrorLocation = Input(UInt(log2Ceil(width + 1).W))
    val injectEccError = Input(Bool())
    val eccErrorLocation = Input(UInt(calcCodeBits(width + 1).W))
